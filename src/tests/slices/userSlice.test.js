...
import user, { setCurrentUser, selectCurrentUser, isLoading } from '../../slices/usersSlice';
...
describe('userSlice', () => {
  ...
  it('should return loading state as boolean', () => {
    const selected = isLoading({ user: { loading: false } })
    expect(typeof(selected)).toEqual('boolean');
  });

  it('should return current user state', () => {
    const selected = selectCurrentUser({ user: { currentUser: userData } })
    expect(selected).toEqual(userData);
  });
});
